...
public class StubbedHttp2RequestHandler implements HttpHandler {
    ...
    public void handleRequest(final HttpServerExchange exchange) throws Exception {
        ...
        responseBodyResolver.resolve(responseSpec.getBody(), context).map(template -> {
            ...
            responseSpec.getHeaders().forEach((header, val) -> {
                final String value = (String) TemplateRuntime.eval(val, context);
                exchange.getResponseHeaders().put(Headers.fromCache(header), value);
            });
